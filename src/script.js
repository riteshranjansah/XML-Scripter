...
function insertContentIntoEditor(content) {
    ...
    editor.scrollToRow(editor.getCursorPosition().row + 10, true);
}
...
function makeOption(content, make, { value = false } = {}) {
    ...
    for (let j = 0; j < lines.length; j++) {
        ...
        if (matchLabelChecked) {
            if (match1) {
                firstPart = match1[1].trim();
                remainingPart = match1[2].trim();
            } else if (match2) {
                firstPart = match2[1].trim();
                remainingPart = match2[2].trim();
            } else {
...
function getCommentRowCol(content) {
    ...
    const rowRegex = /\[\s*(row|ROW|rows|ROWS)\s*\]/;
    const colRegex = /\[\s*(col|COL|cols|COLS|column|COLUMN|columns|COLUMNS)\s*\]/;
    const choiceRegex = /\[\s*(choice|CHOICE|choices|CHOICES)\s*\]/;
...
function makeRadio(content) {
    ...
    console.log(`qcomment: ${qcomment}`);
    console.log(`remainingContents: ${remainingContents}`);
...
function makeTextArea(content) {
    ...
    if (row.length > 0 && col.length > 0 && choice.length > 0) {
        ...
        const { options: coptions } = makeOption(col, 'col', { value: false });
        const { options: choptions } = makeOption(choice, 'row', { value: false });
        finalOutput = `<textarea\nlabel="${qlabel}" optional="0" ss:listDisplay="1" height="10" width="50" ${qAttributes}>\n  <title>${qtitle}</title>\n  <comment>${qcomment}</comment>\n${choptions}\n${roptions}\n${coptions}\n</textarea>\n<suspend/>`;
    } else if (row.length > 0 && col.length > 0 && choice.length == 0) {
        const { options: roptions } = makeOption(row, 'row', { value: false });
        const { options: choptions } = makeOption(col, 'row', { value: false });
        finalOutput = `<textarea\nlabel="${qlabel}" optional="0" ss:listDisplay="1" height="10" width="50" ${qAttributes}>\n  <title>${qtitle}</title>\n  <comment>${qcomment}</comment>\n${choptions}\n${roptions}\n</textarea>\n<suspend/>`;
...
function makeNumber(content) {
    ...
    if (row.length > 0 && col.length > 0 && choice.length > 0) {
        ...
        const { options: coptions } = makeOption(col, 'col', { value: false });
        const { options: choptions } = makeOption(choice, 'row', { value: false });
        finalOutput = `<number\nlabel="${qlabel}" optional="0" size="3" ss:listDisplay="0" verify="range(1,100)" ss:preText="" ss:postText="" ${qAttributes}>\n  <title>${qtitle}</title>\n  <comment>${qcomment}</comment>\n${choptions}\n${roptions}\n${coptions}\n</number>\n<suspend/>`;
    } else if (row.length > 0 && col.length > 0 && choice.length == 0) {
        const { options: roptions } = makeOption(row, 'row', { value: false });
        const { options: choptions } = makeOption(col, 'row', { value: false });
        finalOutput = `<number\nlabel="${qlabel}" optional="0" size="3" ss:listDisplay="0" verify="range(1,100)" ss:preText="" ss:postText="" ${qAttributes}>\n  <title>${qtitle}</title>\n  <comment>${qcomment}</comment>\n${choptions}\n${roptions}\n</number>\n<suspend/>`;
...
function generateXML() {
    ...
    content = content.replace(/(<br\s*\/?>\s*<br\s*\/?>)/g, '').replace(/ +/g, ' ').trim();
    const lines = content ? content.split('\n') : [content];
    const radioRegex = /\[\s*(single|SINGLE|HIDDEN)\s*\]/;
    const checkboxRegex = /\[\s*(multi|MULTI|multiple|MULTIPLE)\s*\]/;
    const numberRegex = /\[\s*(numeric|NUMERIC)\s*\]/;
    const textRegex = /\[\s*(openend|OPENEND|open-end|OPEN-END)\s*\]/;
    const selectRegex = /\[\s*(dropdown|DROPDOWN|drop-down|DROP-DOWN)\s*\]/;
    const htmlRegex = /\[\s*(intro|INTRO)\s*\]/;
    let question = '';
    let currentType = '';
